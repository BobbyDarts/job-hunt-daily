...
const unvisitedSites = computed(() => props.splitCategorySites(props.category).unvisited);
const visitedSites = computed(() => props.splitCategorySites(props.category).visited);
</script>
...
<template>
    <section
        ...
        <!-- Header -->
        <div class="bg-linear-to-r from-primary/5 to-primary/10 px-4 py-3 sm:px-6 sm:py-4 border-b border-border/30">
            ...
            <Progress :model-value="props.getCategoryProgress(props.category)" class="h-1.5 bg-primary/10" />
        </div>

        <!-- Sites List -->
        <ScrollArea class="overflow-hidden bg-card/50" :style="{ height: `${props.maxHeightRem * 4.25}rem` }">
            <div class="flex flex-col gap-2 p-4 sm:p-6 pt-3">
                <!-- Unvisited sites first -->
                <JobSiteButton v-for="site in unvisitedSites" :key="site.url" :site="site"
                    :atsInfo="props.getATS ? props.getATS(site) : undefined" @click="props.onSiteClick" />

                <!-- Visited sites at the bottom -->
                <JobSiteButton v-for="site in visitedSites" :key="site.url" :site="site"
                    :atsInfo="props.getATS ? props.getATS(site) : undefined" variant="visited"
