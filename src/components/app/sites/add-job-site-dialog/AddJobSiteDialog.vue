<!-- // /src/components/app/sites/add-job-site-dialog/AddJobSiteDialog.vue -->

<script setup lang="ts">
import { ref, watch, computed, nextTick } from "vue";

import { ATSSelect, CategorySelect } from "@/components/app/lib";
import { AddCategoryInline } from "@/components/app/sites";
import { Button } from "@/components/ui/button";
import {
  Dialog,
  DialogContent,
  DialogDescription,
  DialogFooter,
  DialogHeader,
  DialogTitle,
} from "@/components/ui/dialog";
import { Input } from "@/components/ui/input";
import { Label } from "@/components/ui/label";
import { ScrollArea } from "@/components/ui/scroll-area";
import { Textarea } from "@/components/ui/textarea";
import { useJobSites } from "@/composables/data";
import type { ATSType } from "@/types";

export interface Props {
  open: boolean;
  categoryId?: string | null;
}

const props = defineProps<Props>();

const emit = defineEmits<{
  "update:open": [value: boolean];
}>();

const { addSite, getSiteByUrl } = useJobSites();

// -------------------------
// Form state
// -------------------------

const defaultFormData = () => ({
  name: "",
  url: "",
  categoryId: "",
  atsType: "" as ATSType | "",
  notes: "",
});

const formData = ref(defaultFormData());
const urlError = ref("");

const isValid = computed(
  () =>
    formData.value.name.trim() &&
    formData.value.url.trim() &&
    formData.value.categoryId &&
    !urlError.value,
);

// -------------------------
// Validation
// -------------------------

function validateUrl() {
  const url = formData.value.url.trim();
  if (!url) {
    urlError.value = "";
    return;
  }

  try {
    new URL(url);
  } catch {
    urlError.value = "Please enter a valid URL";
    return;
  }

  const existing = getSiteByUrl(url);
  if (existing) {
    urlError.value = `URL already exists as "${existing.name}"`;
    return;
  }

  urlError.value = "";
}

// -------------------------
// Reset
// -------------------------

function resetForm() {
  formData.value = defaultFormData();
  urlError.value = "";
}

watch(
  () => props.open,
  isOpen => {
    if (isOpen) {
      resetForm();
      if (props.categoryId) {
        formData.value.categoryId = props.categoryId;
      }
    }
  },
  { immediate: true },
);

watch(
  () => props.categoryId,
  id => {
    if (props.open && id) {
      formData.value.categoryId = id;
    }
  },
);

// -------------------------
// Focus
// -------------------------

const nameInputRef = ref<InstanceType<typeof Input> | null>(null);

watch(
  () => props.open,
  async isOpen => {
    if (isOpen) {
      await nextTick();
      setTimeout(() => {
        nameInputRef.value?.$el?.focus();
      }, 50);
    }
  },
);

// -------------------------
// Submit / Cancel
// -------------------------

async function handleSubmit() {
  validateUrl();
  if (!isValid.value) return;

  await addSite({
    name: formData.value.name.trim(),
    url: formData.value.url.trim(),
    categoryId: formData.value.categoryId,
    atsType: formData.value.atsType || undefined,
    notes: formData.value.notes.trim() || undefined,
  });

  emit("update:open", false);
  resetForm();
}

function handleCancel() {
  emit("update:open", false);
  resetForm();
}
</script>

<template>
  <Dialog :open="open" @update:open="emit('update:open', $event)">
    <DialogContent class="sm:max-w-125 p-0">
      <DialogHeader class="px-6 pt-6 pb-4">
        <DialogTitle>Add Job Site</DialogTitle>
        <DialogDescription>Add a new job site to your list</DialogDescription>
      </DialogHeader>

      <ScrollArea class="h-[60vh] px-6">
        <div class="grid gap-4 pr-4">
          <div class="grid gap-2">
            <Label for="site-name">Name *</Label>
            <Input
              data-testid="site-name-input"
              id="site-name"
              ref="nameInputRef"
              v-model="formData.name"
              placeholder="e.g., Acme Corp Careers"
              @keyup.enter="handleSubmit"
            />
          </div>

          <div class="grid gap-2">
            <Label for="site-url">URL *</Label>
            <Input
              data-testid="site-url-input"
              id="site-url"
              v-model="formData.url"
              placeholder="https://..."
              type="url"
              @blur="validateUrl"
              @keyup.enter="handleSubmit"
            />
            <p v-if="urlError" class="text-sm text-destructive">
              {{ urlError }}
            </p>
          </div>

          <div class="grid gap-2">
            <Label for="site-category">Category *</Label>
            <AddCategoryInline @added="formData.categoryId = $event">
              <CategorySelect
                id="site-category"
                v-model="formData.categoryId"
                placeholder="Select a category"
                :inside-dialog="true"
                class="flex-1"
              />
            </AddCategoryInline>
          </div>

          <div class="grid gap-2">
            <Label for="site-ats">ATS Type</Label>
            <ATSSelect
              id="site-ats"
              v-model="formData.atsType"
              placeholder="None"
              :inside-dialog="true"
              show-all-option
            />
          </div>

          <div class="grid gap-2">
            <Label for="site-notes">Notes</Label>
            <Textarea
              id="site-notes"
              v-model="formData.notes"
              placeholder="Add any notes about this site..."
              class="resize-none min-h-24"
              rows="3"
            />
          </div>
        </div>
      </ScrollArea>

      <DialogFooter class="px-6 pb-6 pt-4">
        <Button variant="outline" @click="handleCancel">Cancel</Button>
        <Button :disabled="!isValid" @click="handleSubmit">Add Site</Button>
      </DialogFooter>
    </DialogContent>
  </Dialog>
</template>
