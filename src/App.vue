    ...
    <ScrollArea class="flex-1 h-full">
      <div class="px-4 py-4 sm:px-6 sm:py-6">
        <div class="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
          <section 
            v-for="(category, _) in sortedCategories" 
            :key="category.name"
          ...
          >
            ...
            <ScrollArea 
              ...
              :style="{ height: `${maxCategoryHeight * 4.25}rem` }"
            >
              <div class="flex flex-col gap-2 p-4 sm:p-6 pt-3">
                <!-- Unvisited sites first -->
                <button
                  v-for="site in splitCategorySites(category).unvisited"
                  :key="site.url"
                ...
                >
                  <span class="truncate pr-2">{{ site.name }}</span>
                </button>
                
                <!-- Visited sites at the bottom -->
                <button
                  v-for="site in splitCategorySites(category).visited"
                  :key="site.url"
                  @click="handleSiteClick(site.url)"
